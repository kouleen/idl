...
struct SystemMenuResponse {
    ...
    4: optional i32 orderNum                // 显示顺序
    5: string path                          // 路由地址
    6: string component                     // 组件路径
    7: string query                         // 路由参数
    8: string routeName                     // 路由名称
    9: optional i8 isFrame                  // 是否为外链（0否 1是）
    10: optional i8 isCache                 // 是否缓存（0不缓存 1缓存）
    11: optional i8 menuType                // 菜单类型（1目录 2菜单 3按钮）
    12: optional i8 visible                 // 是否隐藏（0隐藏 1显示）
